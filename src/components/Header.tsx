      ...
      <div className="mx-auto flex h-20 max-w-[1280px] items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          {navLinks.map((link) =>
            link.dropdown ? (
              ...
              >
                <Link
                  href={link.href}
                  className="flex items-center gap-1 text-[14.5px] font-medium text-slate-700 transition-colors hover:text-[#0052FF]"
                >
                  {link.label}
                  <ChevronDown className={`h-3.5 w-3.5 transition-transform duration-200 ${dropdownOpen ? "rotate-180 text-[#0052FF]" : "text-slate-400"}`} />
                </Link>
                {dropdownOpen && (
                  <div className="absolute left-0 top-full min-w-[240px] rounded-2xl border border-slate-100 bg-white p-2 shadow-xl animate-in fade-in slide-in-from-top-2 duration-200">
                    {link.dropdown.map((item) => (
                      <Link
                        key={item.href}
                        href={item.href}
                        className="block rounded-xl px-3.5 py-2.5 text-[14px] font-medium text-slate-700 transition-colors hover:bg-blue-50/70 hover:text-[#0052FF]"
                      >
                        {item.label}
                      </Link>
                    ))}
              ...
              <Link
                key={link.href}
                href={link.href}
                className="text-[14.5px] font-medium text-slate-700 transition-colors hover:text-[#0052FF]"
              >
                {link.label}
              </Link>
            )
        ...
        <div className="hidden items-center gap-3 md:flex">
          <a
            ...
            className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50/80 px-4 py-2 text-xs font-semibold text-slate-700 transition-all hover:border-blue-200 hover:bg-blue-50 hover:text-[#0052FF]"
            aria-label="Call clinic directly"
          >
            <Phone className="h-3.5 w-3.5 text-[#0052FF]" />
            <span className="hidden xl:inline">+91 94801 66770</span>
